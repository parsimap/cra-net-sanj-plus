...
import { editModeChanged, provinceChanged } from "../../features/appSlice";
...
function Drawer() {
  ...
  const { service, operator, editMode } = useSelector((state: RootState) => state.app);
  ...
  return <>
    <MuiPersistentDrawer scroll={true} open={true} width={410} closeTitle={"نقشه"} openTitle={"اطلاعات"}>
      <Box>
        {editMode ?
          <Edit token={token} /> :
          <MainContent service={service} operator={operator} tabs={tabs} areaInfo={areaInfo}
                       navigate={navigate} />}
      </Box>
    </MuiPersistentDrawer>
  </>;
